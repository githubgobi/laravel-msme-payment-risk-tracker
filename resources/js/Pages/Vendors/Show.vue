<template>
  <AppLayout :title="vendor.name" subtitle="Vendor detail — edit classification and verify Udyam">
    <Head :title="vendor.name" />

    <!-- Breadcrumb -->
    <div class="flex items-center gap-2 mb-5 text-sm">
      <Link :href="route('vendors.index')" class="text-gray-500 hover:text-gray-700">Vendors</Link>
      <span class="text-gray-300">/</span>
      <span class="text-gray-900 font-medium">{{ vendor.name }}</span>
      <span :class="['inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ml-2', categoryBadgeClass(vendor.category)]">
    ...
    <div v-if="$page.props.flash?.success"
         class="mb-5 rounded-lg bg-green-50 border border-green-200 p-4 flex items-start gap-3">
